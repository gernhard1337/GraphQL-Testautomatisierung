...
Die Ergebnisse der Experimente sind im~\href{https://git.informatik.tu-cottbus.de/sst/abschlussarbeiten/master/lorenz_tom/graphql-integrationstest-ma/-/tree/main/experiment/toy-experiment}{GitLab} zu finden.
...
Bemerkenswert ist, dass das Property-based Tool wesentlich mehr Queries benötigte, um eine zufriedenstellende Coverage zu erreichen.
...
Hierzu wurden für jeden Pfad 5 Testqueries entwickelt.
Es war somit möglich, alle 11 Fehler zu finden.
Dabei wurden zwei perfekte Queries ermittelt.
Beide Queries sind in Listing~\ref{q1} und Listing~\ref{q2} gezeigt. \\
...
Mithilfe dieser Queries kann jeder der 11 entdeckten Fehler gefunden werden.
Dies liegt auch daran, dass der Argumentgenerator entsprechend angepasst wurde und nur valide IDs produziert hat.
So war es sehr wahrscheinlich, dass eine ID, die generiert wird, mindestens in einer der 5 erstellten Queries zur zugrundeliegenden Datenstruktur gepasst hat
...
Die Fehler wurden gefunden durch die Queries aus Listing~\ref{i1},~\ref{i2},~\ref{i3} und~\ref{i4}.
...
Dafür waren die Queries \hyperref[query1]{Query 1}, \hyperref[query2]{Query 2}, \hyperref[query3]{Query 3} und \hyperref[query4]{Query 4} ausreichend.
Es ist zu beobachten, dass die Queries wesentlich länger sind als jene, die vom \textit{Property-based Testtool} erstellt wurden.
...
Da die Querygenerierung stets im Query-Knoten beginnt und die PrimePaths gefunden werden sollen, ergeben sich in diesem speziellen Schema sehr viele ähnliche Queries.
...
Die PrimePfad-Abdeckung wird auf unbeschnittenen Graphen ausgeführt.
...
die Definition~\ref{defedgecov} erfüllt sein, dass jedes Paar von \verb+(Type, objectField)+ berücksichtigt ist.
...
Es kann keine Aussage darüber gemacht werden, ob die generierten Queries von \textit{Property-based Testing}~\cite{property-based-testing} diese Abdeckung erfüllen, denn bei der Querygenerierung spielt es keine Rolle, ob dieses Kriterium erreicht wird.
...
Hierdurch konnte gezeigt werden, dass die generierten Queries in Teilen die Abdeckung auch tatsächlich umsetzen.
...
Die Berechnung der Queries geschah auf einem hardwaretechnisch ähnlichem Level wie in Property-based Testing angewandt~\cite[vgl. Experimental Setup]{property-based-testing}.
